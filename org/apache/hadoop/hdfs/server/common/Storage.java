...
public abstract class Storage extends StorageInfo {
  ...
  public void removeStorageDir(int idx) {
    storageDirs.remove(idx);
  }

  /**
   * Gets the storage directory index in the storage list
   * @param dir Storage directory absolute path
   * @return Index of storage directory
   */
  public int getStorageDirIndex(String dir) {
    for(int idx = 0; idx < storageDirs.size(); idx++){
      StorageDirectory sd = storageDirs.get(idx);
      if (sd.getCurrentDir().getParent().equals(dir))
        return idx;
    }
    return -1;
  }
